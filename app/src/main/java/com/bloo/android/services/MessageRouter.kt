...
class MessageRouter private constructor(
        ...
        fun getInstance(context: Context, mesh: BluetoothMeshService): MessageRouter {
            return INSTANCE ?: synchronized(this) {
                ...
                MessageRouter(context.applicationContext, mesh, nostr).also { instance ->
                    ...
                    try {
                        com.bloo.android.favorites.FavoritesPersistenceService.shared.addListener(instance.favoriteListener)
                    } catch (_: Exception) {}
                    INSTANCE = instance
                }
            }
    ...
    private val favoriteListener = object: com.bloo.android.favorites.FavoritesChangeListener {
    ...
    fun sendPrivate(content: String, toPeerID: String, recipientNickname: String, messageID: String) {
        ...
        if (com.bloo.android.nostr.GeohashAliasRegistry.contains(toPeerID)) {
            Log.d(TAG, "Routing PM via Nostr (geohash) to alias ${toPeerID.take(12)}… id=${messageID.take(8)}…")
            val recipientHex = com.bloo.android.nostr.GeohashAliasRegistry.get(toPeerID)
            if (recipientHex != null) {
                // Resolve the conversation's source geohash, so we can send from anywhere
                val sourceGeohash = com.bloo.android.nostr.GeohashConversationRegistry.get(toPeerID)
    ...
    fun sendDeliveryAck(messageID: String, toPeerID: String) {
        ...
        if (com.bloo.android.nostr.GeohashAliasRegistry.contains(toPeerID)) {
            val recipientHex = com.bloo.android.nostr.GeohashAliasRegistry.get(toPeerID)
            if (recipientHex != null) {
                nostr.sendDeliveryAckGeohash(messageID, recipientHex, try { com.bloo.android.nostr.NostrIdentityBridge.getCurrentNostrIdentity(context)!! } catch (_: Exception) { return })
                return
            }
        }
        if (!((mesh.getPeerInfo(toPeerID)?.isConnected == true) && mesh.hasEstablishedSession(toPeerID))) {
    ...
    fun sendFavoriteNotification(toPeerID: String, isFavorite: Boolean) {
        if (mesh.getPeerInfo(toPeerID)?.isConnected == true) {
            val myNpub = try { com.bloo.android.nostr.NostrIdentityBridge.getCurrentNostrIdentity(context)?.npub } catch (_: Exception) { null }
    ...
    private fun canSendViaNostr(peerID: String): Boolean {
        return try {
            ...
            if (peerID.length == 64 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                ...
                val fav = com.bloo.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
                ...
                val fav = com.bloo.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)
                fav?.isMutual == true && fav.peerNostrPublicKey != null
            } else {
                false
            }
