...
import com.bloo.android.mesh.BluetoothPacketBroadcaster
...
class GossipSyncManager(
    ...
    interface Delegate {
        ...
        fun sendPacketToPeer(peerID: String, packet: BlooPacket)
        fun signPacketForBroadcast(packet: BlooPacket): BlooPacket
    }
    ...
    private val messages = LinkedHashMap<String, BlooPacket>()
    // - announcements: only keep latest per sender peerID
    private val latestAnnouncementByPeer = ConcurrentHashMap<String, Pair<String, BlooPacket>>()
    ...
    fun start() {
        ...
        cleanupJob = scope.launch(Dispatchers.IO) {
            while (isActive) {
                try {
                    delay(com.bloo.android.util.AppConstants.Sync.CLEANUP_INTERVAL_MS)
    ...
    fun onPublicPacketSeen(packet: BlooPacket) {
        ...
        if (isBroadcastMessage) {
            ...
            if (age > com.bloo.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS) {
                Log.d(TAG, "Ignoring stale ANNOUNCE (age=${age}ms > ${com.bloo.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS}ms)")
                return
            }
    ...
    private fun sendRequestSync() {
        ...
        val packet = BlooPacket(
            ...
            payload = payload,
            ttl = com.bloo.android.util.AppConstants.SYNC_TTL_HOPS // neighbors only
        )
        // Sign and broadcast
    ...
    private fun sendRequestSyncToPeer(peerID: String) {
        ...
        val packet = BlooPacket(
            ...
            timestamp = System.currentTimeMillis().toULong(),
            payload = payload,
            ttl = com.bloo.android.util.AppConstants.SYNC_TTL_HOPS // neighbor only
        )
    ...
    fun handleRequestSync(fromPeerID: String, request: RequestSyncPacket) {
        ...
        for ((_, pair) in latestAnnouncementByPeer.entries) {
            ...
            if (!mightContain(idBytes)) {
                ...
                val toSend = pkt.copy(ttl = com.bloo.android.util.AppConstants.SYNC_TTL_HOPS)
