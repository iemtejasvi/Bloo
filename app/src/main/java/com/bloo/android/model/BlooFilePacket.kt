...
/**
 * BlooFilePacket: TLV-encoded file transfer payload for BLE mesh.
 * TLVs:
 *  - 0x01: filename (UTF-8)
 ...
 *
 * Note: The outer BlooPacket uses version 2 (4-byte payload length), so this
...
data class BlooFilePacket(
    ...
    fun encode(): ByteArray? {
        try {
            android.util.Log.d("BlooFilePacket", "🔄 Encoding: name=$fileName, size=$fileSize, mime=$mimeType")
        ...
        if (nameBytes.size > 0xFFFF || mimeBytes.size > 0xFFFF) {
                android.util.Log.e("BlooFilePacket", "❌ TLV field too large: name=${nameBytes.size}, mime=${mimeBytes.size} (max: 65535)")
                return null
            }
            if (content.size > 0xFFFF) {
                android.util.Log.d("BlooFilePacket", "📦 Content exceeds 65535 bytes (${content.size}); will be split into multiple CONTENT TLVs")
            } else {
                android.util.Log.d("BlooFilePacket", "📏 TLV sizes OK: name=${nameBytes.size}, mime=${mimeBytes.size}, content=${content.size}")
            }
        ...
        val result = buf.array()
            android.util.Log.d("BlooFilePacket", "✅ Encoded successfully: ${result.size} bytes total")
            return result
        } catch (e: Exception) {
            android.util.Log.e("BlooFilePacket", "❌ Encoding failed: ${e.message}", e)
            return null
        }
    }

    companion object {
        fun decode(data: ByteArray): BlooFilePacket? {
            android.util.Log.d("BlooFilePacket", "🔄 Decoding ${data.size} bytes")
            try {
                var off = 0
                ...
                val result = BlooFilePacket(n, s, m, c)
                android.util.Log.d("BlooFilePacket", "✅ Decoded: name=$n, size=$s, mime=$m, content=${c.size} bytes")
                return result
            } catch (e: Exception) {
                android.util.Log.e("BlooFilePacket", "❌ Decoding failed: ${e.message}", e)
                return null
            }
        }
    }
