...
class BluetoothConnectionManager(
    ...
    private val componentDelegate = object : BluetoothConnectionManagerDelegate {
        override fun onPacketReceived(packet: BlooPacket, peerID: String, device: BluetoothDevice?) {
    ...
    init {
        ...
        try {
            val dbg = com.bloo.android.ui.debug.DebugSettingsManager.getInstance()
            // Role enable/disable
    ...
    fun startServices(): Boolean {
        ...
        // }
            ...
            connectionScope.launch {
                ...
                val dbg = try { com.bloo.android.ui.debug.DebugSettingsManager.getInstance() } catch (_: Exception) { null }
     ...
     */
    fun sendPacketToPeer(peerID: String, packet: BlooPacket): Boolean {
    ...
    override fun onPowerModeChanged(newMode: PowerManager.PowerMode) {
        ...
        connectionScope.launch {
            ...
            val serverEnabled = try { com.bloo.android.ui.debug.DebugSettingsManager.getInstance().gattServerEnabled.value } catch (_: Exception) { true }
            if (serverEnabled) {
            ...
            if (wasUsingDutyCycle != nowUsingDutyCycle) {
                ...
                val clientEnabled = try { com.bloo.android.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value } catch (_: Exception) { true }
                if (clientEnabled) {
            ...
            try {
                val maxServer = com.bloo.android.ui.debug.DebugSettingsManager.getInstance().maxServerConnections.value
...
interface BluetoothConnectionManagerDelegate {
    fun onPacketReceived(packet: BlooPacket, peerID: String, device: BluetoothDevice?)
