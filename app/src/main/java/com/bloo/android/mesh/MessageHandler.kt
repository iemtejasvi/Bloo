...
class MessageHandler(private val myPeerID: String, private val appContext: android.content.Context) {
    ...
    suspend fun handleNoiseEncrypted(routed: RoutedPacket) {
        ...
        try {
            ...
            val noisePayload = com.bloo.android.model.NoisePayload.decode(decryptedData)
            ...
            when (noisePayload.type) {
                com.bloo.android.model.NoisePayloadType.PRIVATE_MESSAGE -> {
                    // Decode TLV private message exactly like iOS
                    val privateMessage = com.bloo.android.model.PrivateMessagePacket.decode(noisePayload.data)
                    if (privateMessage != null) {
                        ...
                        // Create BlooMessage - preserve source packet timestamp
                ...
                com.bloo.android.model.NoisePayloadType.FILE_TRANSFER -> {
                    // Handle encrypted file transfer; generate unique message ID
                    val file = com.bloo.android.model.BlooFilePacket.decode(noisePayload.data)
                    if (file != null) {
                        Log.d(TAG, "🔓 Decrypted encrypted file from $peerID: name='${file.fileName}', size=${file.fileSize}, mime='${file.mimeType}'")
                        val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                        val savedPath = com.bloo.android.features.file.FileUtils.saveIncomingFile(appContext, file)
                        val message = BlooMessage(
                            id = uniqueMsgId,
                            sender = delegate?.getPeerNickname(peerID) ?: "Unknown",
                            content = savedPath,
                            type = com.bloo.android.features.file.FileUtils.messageTypeForMime(file.mimeType),
                            timestamp = java.util.Date(packet.timestamp.toLong()),
                            isRelay = false,
                            isPrivate = true,
                            recipientNickname = delegate?.getMyNickname(),
                ...
                com.bloo.android.model.NoisePayloadType.DELIVERED -> {
                ...
                com.bloo.android.model.NoisePayloadType.READ_RECEIPT -> {
    ...
    private suspend fun sendDeliveryAck(messageID: String, senderPeerID: String) {
        try {
            ...
            val ackPayload = com.bloo.android.model.NoisePayload(
                type = com.bloo.android.model.NoisePayloadType.DELIVERED,
    ...
    suspend fun handleAnnounce(routed: RoutedPacket): Boolean {
        ...
        if (age > com.bloo.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS) {
            Log.w(TAG, "Ignoring stale ANNOUNCE from ${peerID.take(8)} (age=${age}ms > ${com.bloo.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS}ms)")
            return false
        }
        ...
        try {
            ...
            if (response != null) {
                ...
                val responsePacket = BlooPacket(
                    ...
                    payload = response,
                    signature = null,
                    ttl = com.bloo.android.util.AppConstants.MESSAGE_TTL_HOPS // Same TTL as iOS
                )
    ...
    private suspend fun handleBroadcastMessage(routed: RoutedPacket) {
        ...
        try {
            ...
            val isFileTransfer = com.bloo.android.protocol.MessageType.fromValue(packet.type) == com.bloo.android.protocol.MessageType.FILE_TRANSFER
            val file = com.bloo.android.model.BlooFilePacket.decode(packet.payload)
            if (file != null) {
                if (isFileTransfer) {
     ...
     */
    private suspend fun handlePrivateMessage(packet: BlooPacket, peerID: String) {
        try {
    ...
    private fun handleFavoriteNotificationFromMesh(content: String, fromPeerID: String) {
        try {
            ...
            if (noiseKey != null) {
                com.bloo.android.favorites.FavoritesPersistenceService.shared.updatePeerFavoritedUs(noiseKey, isFavorite)
                if (npub != null) {
                    // Index by noise key and current mesh peerID for fast Nostr routing
                    com.bloo.android.favorites.FavoritesPersistenceService.shared.updateNostrPublicKey(noiseKey, npub)
                    com.bloo.android.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(fromPeerID, npub)
                }
                ...
                val sys = com.bloo.android.model.BlooMessage(
                    sender = "system",
...
interface MessageHandlerDelegate {
    ...
    // Packet operations
    fun sendPacket(packet: BlooPacket)
    fun relayPacket(routed: RoutedPacket)
    fun getBroadcastRecipient(): ByteArray
    
    // Cryptographic operations
    fun verifySignature(packet: BlooPacket, peerID: String): Boolean
    ...
    // Callbacks
    fun onMessageReceived(message: BlooMessage)
