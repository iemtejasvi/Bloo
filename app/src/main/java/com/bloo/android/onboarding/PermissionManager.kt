package com.bloo.android.onboarding
...
/**
 * Centralized permission management for bloo app
    ...
    fun getCategorizedPermissions(): List<PermissionCategory> {
        ...
        categories.add(
            PermissionCategory(
                ...
                description = "Required to discover bloo users via Bluetooth",
                permissions = bluetoothPermissions,
                isGranted = bluetoothPermissions.all { isPermissionGranted(it) },
                systemDescription = "Allow bloo to connect to nearby devices"
            )
        )
        ...
        categories.add(
            PermissionCategory(
                ...
                description = "Required by Android to discover nearby bloo users via Bluetooth",
                permissions = locationPermissions,
                isGranted = locationPermissions.all { isPermissionGranted(it) },
                systemDescription = "bloo needs this to scan for nearby devices"
            )
        )
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            categories.add(
                PermissionCategory(
                    ...
                    systemDescription = "Allow bloo to send you notifications"
                )
            )
        }
        ...
        if (isBatteryOptimizationSupported()) {
            categories.add(
                PermissionCategory(
                    ...
                    description = "Disable battery optimization to ensure bloo runs reliably in the background and maintains mesh network connections",
                    permissions = listOf("BATTERY_OPTIMIZATION"), // Custom identifier
                    isGranted = isBatteryOptimizationDisabled(),
                    systemDescription = "Allow bloo to run without battery restrictions"
                )
            )
        }
