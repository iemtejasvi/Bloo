package com.bloo.android.protocol
...
data class BlooPacket(
    ...
    fun toBinaryDataForSigning(): ByteArray? {
        ...
        val unsignedPacket = BlooPacket(
            ...
            ttl = com.bloo.android.util.AppConstants.SYNC_TTL_HOPS // Use fixed TTL=0 for signing to ensure relay compatibility
        )
        return BinaryProtocol.encode(unsignedPacket)
    }

    companion object {
        fun fromBinaryData(data: ByteArray): BlooPacket? {
...
object BinaryProtocol {
    ...
    fun encode(packet: BlooPacket): ByteArray? {
        try {
    ...
    fun decode(data: ByteArray): BlooPacket? {
     ...
     */
    private fun decodeCore(raw: ByteArray): BlooPacket? {
        try {
