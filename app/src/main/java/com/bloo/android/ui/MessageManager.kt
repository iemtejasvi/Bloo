...
class MessageManager(private val state: ChatState) {
    ...
    private val MESSAGE_DEDUP_TIMEOUT = com.bloo.android.util.AppConstants.UI.MESSAGE_DEDUP_TIMEOUT_MS // 30 seconds
    private val SYSTEM_EVENT_DEDUP_TIMEOUT = com.bloo.android.util.AppConstants.UI.SYSTEM_EVENT_DEDUP_TIMEOUT_MS // 5 seconds
    
    // MARK: - Public Message Management
    
    fun addMessage(message: BlooMessage) {
    ...
    fun addChannelMessage(channel: String, message: BlooMessage) {
        ...
        val viewingGeohashChannel = try {
            if (channel.startsWith("geo:")) {
                ...
                selected is com.bloo.android.geohash.ChannelID.Location && selected.channel.geohash.equals(geo, ignoreCase = true)
            } else false
    ...
    fun addPrivateMessage(peerID: String, message: BlooMessage) {
    ...
    fun addPrivateMessageNoUnread(peerID: String, message: BlooMessage) {
        val currentPrivateChats = state.getPrivateChatsValue().toMutableMap()
        if (!currentPrivateChats.containsKey(peerID)) {
            currentPrivateChats[peerID] = mutableListOf()
        }
     ...
     */
    fun generateMessageKey(message: BlooMessage): String {
