...
import com.bloo.android.R
import androidx.compose.ui.text.font.FontWeight
import androidx.compose.ui.text.input.ImeAction
import androidx.compose.ui.unit.dp
import androidx.compose.ui.unit.sp
import com.bloo.android.core.ui.utils.singleOrTripleClickable
import com.bloo.android.geohash.LocationChannelManager.PermissionState
...
fun TorStatusDot(
    ...
    val torStatus by com.bloo.android.net.TorManager.statusFlow.collectAsState()
    
    if (torStatus.mode != com.bloo.android.net.TorMode.OFF) {
        val dotColor = when {
...
fun PeerCounter(
    ...
    isConnected: Boolean,
    selectedLocationChannel: com.bloo.android.geohash.ChannelID?,
    ...
    Row(
        verticalAlignment = Alignment.CenterVertically,
        modifier = modifier.clickable { onClick() }.padding(end = 8.dp) // Added right margin to match "bloo" logo spacing
    ) {
        Icon(
            imageVector = Icons.Default.Group,
            contentDescription = when (selectedLocationChannel) {
                is com.bloo.android.geohash.ChannelID.Location -> stringResource(R.string.cd_geohash_participants)
...
private fun PrivateChatHeader(
    ...
    val isMutualFavorite = remember(peerID, peerNicknames) {
        try {
            ...
            if (peerID.length == 64 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                ...
                com.bloo.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKeyBytes)?.isMutual == true
            } else if (peerID.length == 16 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                com.bloo.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)?.isMutual == true
            } else false
    ...
    val titleText: String = if (isNostrDM) {
        ...
        val (conversationGeohash, baseName) = try {
            val repoField = com.bloo.android.ui.GeohashViewModel::class.java.getDeclaredField("repo")
            repoField.isAccessible = true
            val repo = repoField.get(viewModel.geohashViewModel) as com.bloo.android.nostr.GeohashRepository
            val gh = repo.getConversationGeohash(peerID) ?: "geohash"
            val fullPubkey = com.bloo.android.nostr.GeohashAliasRegistry.get(peerID) ?: ""
        ...
        peerNicknames[peerID] ?: run {
            val titleFromFavorites = try {
                if (peerID.length == 64 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                    val noiseKeyBytes = peerID.chunked(2).map { it.toInt(16).toByte() }.toByteArray()
                    com.bloo.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKeyBytes)?.peerNickname
                } else if (peerID.length == 16 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                    com.bloo.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)?.peerNickname
                } else null
...
private fun MainHeader(
    ...
    val bookmarksStore = remember { com.bloo.android.geohash.GeohashBookmarksStore.getInstance(context) }
        ...
        Row(
            ...
            Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.padding(end = 4.dp)) {
                ...
                val currentGeohash: String? = when (val sc = selectedLocationChannel) {
                    is com.bloo.android.geohash.ChannelID.Location -> sc.channel.geohash
                    else -> null
                }
...
private fun LocationChannelsButton(
    ...
    val (badgeText, badgeColor) = when (selectedChannel) {
        ...
        is com.bloo.android.geohash.ChannelID.Location -> {
            val geohash = (selectedChannel as com.bloo.android.geohash.ChannelID.Location).channel.geohash
