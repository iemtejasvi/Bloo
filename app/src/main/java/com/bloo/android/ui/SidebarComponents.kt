            ...
            Row(
                ...
                IconButton(
                    ...
                    Icon(
                        ...
                        contentDescription = stringResource(com.bloo.android.R.string.cd_leave_channel),
...
fun PeopleSection(
    ...
    Column(modifier = modifier) {
        ...
        sortedPeers.forEach { pid ->
            ...
            val (b, _) = com.bloo.android.ui.splitSuffix(dn)
            if (b != "You") baseNameCounts[b] = (baseNameCounts[b] ?: 0) + 1
        }

        // Offline favorites (exclude ones mapped to connected)
        val offlineFavorites = com.bloo.android.favorites.FavoritesPersistenceService.shared.getOurFavorites()
        ...
        sortedPeers.forEach { peerID ->
            ...
            val (bName, _) = com.bloo.android.ui.splitSuffix(displayName)
        ...
        offlineFavorites.forEach { fav ->
            ...
            val nostrConvKey: String? = try {
                val npubOrHex = com.bloo.android.favorites.FavoritesPersistenceService.shared.findNostrPubkey(fav.peerNoisePublicKey)
                if (npubOrHex != null) {
                    val hex = if (npubOrHex.startsWith("npub")) {
                        val (hrp, data) = com.bloo.android.nostr.Bech32.decode(npubOrHex)
            ...
            val dn = peerNicknames[favPeerID] ?: fav.peerNickname
            val (bName, _) = com.bloo.android.ui.splitSuffix(dn)
            val showHash = (baseNameCounts[bName] ?: 0) > 1
...
private fun PeerItem(
    ...
    val (baseNameRaw, suffixRaw) = com.bloo.android.ui.splitSuffix(displayName)
    ...
    Row(
        ...
        if (hasUnreadDM) {
            ...
            Icon(
                ...
                contentDescription = stringResource(com.bloo.android.R.string.cd_unread_message),
            ...
            if (showNostrGlobe) {
                ...
                Icon(
                    ...
                    contentDescription = stringResource(com.bloo.android.R.string.cd_reachable_via_nostr),
                    modifier = Modifier.size(16.dp),
                ...
                Icon(
                    ...
                    contentDescription = stringResource(com.bloo.android.R.string.cd_offline_favorite),
                    modifier = Modifier.size(16.dp),
                    tint = Color.Gray
                )
            } else {
