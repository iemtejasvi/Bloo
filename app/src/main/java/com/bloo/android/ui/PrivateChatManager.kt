...
class PrivateChatManager(
    ...
    private val unreadReceivedMessages = mutableMapOf<String, MutableList<BlooMessage>>()
    ...
    fun handleIncomingPrivateMessage(message: BlooMessage) {
        handleIncomingPrivateMessage(message, suppressUnread = false)
    }

    fun handleIncomingPrivateMessage(message: BlooMessage, suppressUnread: Boolean) {
    ...
    private fun consolidateNostrTempConversationIfNeeded(targetPeerID: String) {
        ...
        try {
            ...
            val npub = com.bloo.android.favorites.FavoritesPersistenceService.shared.findNostrPubkey(noiseKeyBytes)
            if (npub != null) {
