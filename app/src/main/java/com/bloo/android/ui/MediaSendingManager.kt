...
import com.bloo.android.model.BlooFilePacket
...
class MediaSendingManager(
    ...
    companion object {
        ...
        private const val MAX_FILE_SIZE = com.bloo.android.util.AppConstants.Media.MAX_FILE_SIZE_BYTES // 50MB limit
    }
    ...
    fun sendVoiceNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            if (toPeerIDOrNull != null) {
                sendPrivateFile(toPeerIDOrNull, filePacket, filePath, BlooMessageType.Audio)
            } else {
                sendPublicFile(channelOrNull, filePacket, filePath, BlooMessageType.Audio)
            }
        } catch (e: Exception) {
    ...
    fun sendImageNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            if (toPeerIDOrNull != null) {
                sendPrivateFile(toPeerIDOrNull, filePacket, filePath, BlooMessageType.Image)
            } else {
                sendPublicFile(channelOrNull, filePacket, filePath, BlooMessageType.Image)
            }
        } catch (e: Exception) {
    ...
    fun sendFileNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            val mimeType = try { 
                com.bloo.android.features.file.FileUtils.getMimeTypeFromExtension(file.name) 
            } catch (_: Exception) { 
            ...
            val messageType = when {
                mimeType.lowercase().startsWith("image/") -> BlooMessageType.Image
                mimeType.lowercase().startsWith("audio/") -> BlooMessageType.Audio
                else -> BlooMessageType.File
            }
    ...
    private fun sendPrivateFile(
        toPeerID: String,
        filePacket: BlooFilePacket,
        filePath: String,
        messageType: BlooMessageType
    ) {
        ...
        messageManager.updateMessageDeliveryStatus(
            msg.id,
            com.bloo.android.model.DeliveryStatus.PartiallyDelivered(0, 100)
        )
     ...
     */
    fun handleTransferProgressEvent(evt: com.bloo.android.mesh.TransferProgressEvent) {
        val msgId = synchronized(transferMessageMap) { transferMessageMap[evt.transferId] }
        if (msgId != null) {
            if (evt.completed) {
                messageManager.updateMessageDeliveryStatus(
                    msgId,
                    com.bloo.android.model.DeliveryStatus.Delivered(to = "mesh", at = java.util.Date())
                )
                synchronized(transferMessageMap) {
                ...
                messageManager.updateMessageDeliveryStatus(
                    msgId,
                    com.bloo.android.model.DeliveryStatus.PartiallyDelivered(evt.sent, evt.total)
                )
            }
        }
    }
