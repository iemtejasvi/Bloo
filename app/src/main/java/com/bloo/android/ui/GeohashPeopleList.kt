        ...
        if (geohashPeople.isEmpty()) {
            ...
            val myHex = remember(selectedLocationChannel) {
                when (val channel = selectedLocationChannel) {
                    is com.bloo.android.geohash.ChannelID.Location -> {
                        try {
                            val identity = com.bloo.android.nostr.NostrIdentityBridge.deriveIdentity(
            ...
            val baseNameCounts = remember(geohashPeople) {
                ...
                geohashPeople.forEach { person ->
                    val (b, _) = com.bloo.android.ui.splitSuffix(person.displayName)
            ...
            orderedPeople.forEach { person ->
                GeohashPersonItem(
                    ...
                    viewModel = viewModel,
                    showHashSuffix = (baseNameCounts[com.bloo.android.ui.splitSuffix(person.displayName).first] ?: 0) > 1,
                    onTap = {
                        if (person.id != myHex) {
...
private fun GeohashPersonItem(
    ...
    Row(
        ...
        val (baseNameRaw, suffixRaw) = com.bloo.android.ui.splitSuffix(person.displayName)
