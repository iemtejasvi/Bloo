...
import com.bloo.android.ui.NotificationTextUtils
...
class MeshDelegateHandler(
    ...
    override fun didUpdatePeerList(peers: List<String>) {
        coroutineScope.launch {
            ...
            runCatching { com.bloo.android.services.MessageRouter.tryGetInstance()?.onPeersUpdated(peers) }
                ...
                if (isNostrAlias || isNoiseHex) {
                    ...
                    val canonical = com.bloo.android.services.ConversationAliasResolver.resolveCanonicalPeerID(
                        ...
                        nostrPubHexForAlias = { alias ->
                            ...
                            if (com.bloo.android.nostr.GeohashAliasRegistry.contains(alias)) {
                                com.bloo.android.nostr.GeohashAliasRegistry.get(alias)
                            } else {
                                // Best-effort: derive pub hex from favorites mapping for mesh nostr_ aliases
                                val prefix = alias.removePrefix("nostr_")
                                val favs = try { com.bloo.android.favorites.FavoritesPersistenceService.shared.getOurFavorites() } catch (_: Exception) { emptyList() }
                                favs.firstNotNullOfOrNull { rel ->
                                    rel.peerNostrPublicKey?.let { s ->
                                        runCatching { com.bloo.android.nostr.Bech32.decode(s) }.getOrNull()?.let { dec ->
                    ...
                    if (canonical != currentPeer) {
                        ...
                        com.bloo.android.services.ConversationAliasResolver.unifyChatsIntoPeer(state, canonical, listOf(currentPeer))
                    ...
                    val favoriteRel = try {
                        ...
                        if (noiseKey != null) {
                            com.bloo.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
                        } else null
                    ...
                    if (favoriteRel?.isMutual == true) {
                        ...
                        if (noiseHex != currentPeer) {
                            com.bloo.android.services.ConversationAliasResolver.unifyChatsIntoPeer(
                                state = state,
            ...
            peers.forEach { pid ->
                try {
                    ...
                    val npub = com.bloo.android.favorites.FavoritesPersistenceService.shared.findNostrPubkey(noiseKey)
                    val tempNostrKey: String? = try {
                        if (npub != null) {
                            val (hrp, data) = com.bloo.android.nostr.Bech32.decode(npub)
    ...
    private fun unifyChatsIntoPeer(targetPeerID: String, keysToMerge: List<String>) {
        com.bloo.android.services.ConversationAliasResolver.unifyChatsIntoPeer(state, targetPeerID, keysToMerge)
    }
     ...
     */
    private fun checkAndTriggerMeshMentionNotification(message: BlooMessage) {
        try {
     ...
     */
    fun getPeerInfo(peerID: String): com.bloo.android.mesh.PeerInfo? {
