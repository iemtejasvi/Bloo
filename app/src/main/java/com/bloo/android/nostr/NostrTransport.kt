...
import com.bloo.android.model.ReadReceipt
import com.bloo.android.model.NoisePayloadType
...
class NostrTransport(
    ...
    companion object {
        ...
        private const val READ_ACK_INTERVAL = com.bloo.android.util.AppConstants.Nostr.READ_ACK_INTERVAL_MS // ~3 per second (0.35s interval like iOS)
    ...
    fun sendPrivateMessage(
        ...
        transportScope.launch {
            try {
                ...
                // Strict: lookup the recipient's current Bloo peer ID using favorites mapping
                val recipientPeerIDForEmbed = try {
                    com.bloo.android.favorites.FavoritesPersistenceService.shared
                ...
                val embedded = NostrEmbeddedBloo.encodePMForNostr(
                    content = content,
                    messageID = messageID,
    ...
    private fun sendNextReadAck() {
        ...
        transportScope.launch {
            try {
                ...
                val ack = NostrEmbeddedBloo.encodeAckForNostr(
    ...
    fun sendDeliveryAckGeohash(
        ...
        transportScope.launch {
            try {
                ...
                val embedded = NostrEmbeddedBloo.encodeAckForNostrNoRecipient(
                    type = NoisePayloadType.DELIVERED,
                    messageID = messageID,
                    senderPeerID = senderPeerID
                )
    ...
    fun sendPrivateMessageGeohash(
        ...
        val geohash = sourceGeohash ?: run {
            val selected = try {
                com.bloo.android.geohash.LocationChannelManager.getInstance(context).selectedChannel.value
            } catch (_: Exception) { null }
            if (selected !is com.bloo.android.geohash.ChannelID.Location) {
        ...
        transportScope.launch {
            try {
                ...
                // Build embedded Bloo packet without recipient peer ID
                val embedded = NostrEmbeddedBloo.encodePMForNostrNoRecipient(
                    content = content,
                    messageID = messageID,
                    senderPeerID = senderPeerID
                ) ?: run {
    ...
    private fun resolveNostrPublicKey(peerID: String): String? {
        try {
            ...
            com.bloo.android.favorites.FavoritesPersistenceService.shared.findNostrPubkeyForPeerID(peerID)?.let { return it }

            // 2) Legacy path: resolve by noise public key association
            val noiseKey = hexStringToByteArray(peerID)
            val favoriteStatus = com.bloo.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
            ...
            if (peerID.length == 16) {
                val fallbackStatus = com.bloo.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)
