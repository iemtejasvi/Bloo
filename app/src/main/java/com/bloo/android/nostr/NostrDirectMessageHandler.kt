package com.bloo.android.nostr
...
import com.bloo.android.services.SeenMessageStore
import com.bloo.android.ui.ChatState
import com.bloo.android.ui.MeshDelegateHandler
import com.bloo.android.ui.PrivateChatManager
...
class NostrDirectMessageHandler(
    ...
    private val dataManager: com.bloo.android.ui.DataManager
) {
    ...
    fun onGiftWrap(giftWrap: NostrEvent, geohash: String, identity: NostrIdentity) {
        scope.launch(Dispatchers.Default) {
            try {
                ...
                if (!content.startsWith("bloo1:")) return@launch

                val base64Content = content.removePrefix("bloo1:")
                val packetData = base64URLDecode(base64Content) ?: return@launch
                val packet = BlooPacket.fromBinaryData(packetData) ?: return@launch

                if (packet.type != com.bloo.android.protocol.MessageType.NOISE_ENCRYPTED.value) return@launch

                val noisePayload = com.bloo.android.model.NoisePayload.decode(packet.payload) ?: return@launch
                val messageTimestamp = Date(giftWrap.createdAt * 1000L)
                val convKey = "nostr_${senderPubkey.take(16)}"
                repo.putNostrKeyMapping(convKey, senderPubkey)
                com.bloo.android.nostr.GeohashAliasRegistry.put(convKey, senderPubkey)
    ...
    private suspend fun processNoisePayload(
        payload: com.bloo.android.model.NoisePayload,
        convKey: String,
        ...
        when (payload.type) {
            com.bloo.android.model.NoisePayloadType.PRIVATE_MESSAGE -> {
                val pm = com.bloo.android.model.PrivateMessagePacket.decode(payload.data) ?: return
            ...
            com.bloo.android.model.NoisePayloadType.FILE_TRANSFER -> {
                ...
                val file = com.bloo.android.model.BlooFilePacket.decode(payload.data)
                if (file != null) {
                    val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                    val savedPath = com.bloo.android.features.file.FileUtils.saveIncomingFile(application, file)
                    val message = BlooMessage(
                        id = uniqueMsgId,
                        sender = senderNickname,
                        content = savedPath,
