...
class FileTransferTest {
    ...
    fun `encode and decode file packet with all fields should preserve data`() {
        ...
        val originalPacket = BlooFilePacket(
            fileName = "test.png",
            mimeType = "image/png",
            fileSize = 1024000,
        ...
        val decoded = BlooFilePacket.decode(encoded!!)
    ...
    fun `replaceFilePathInContent should correctly format content markers for different file types`() {
        ...
        val imageMessage = BlooMessage(
            id = "test1",
            sender = "alice",
            senderPeerID = "12345678",
            content = "/data/user/0/com.bloo.android/files/images/photo.jpg",
            type = BlooMessageType.Image,
            timestamp = Date(System.currentTimeMillis()),
            isPrivate = false
        )

        val audioMessage = BlooMessage(
            id = "test2",
            sender = "bob",
            senderPeerID = "87654321",
            content = "/data/user/0/com.bloo.android/files/audio/voice.amr",
            type = BlooMessageType.Audio,
            timestamp = Date(System.currentTimeMillis()),
            isPrivate = false
        )

        val fileMessage = BlooMessage(
            id = "test3",
            sender = "charlie",
            senderPeerID = "11223344",
            content = "/data/user/0/com.bloo.android/files/documents/document.pdf",
            type = BlooMessageType.File,
            timestamp = Date(System.currentTimeMillis()),
            isPrivate = false
        )
        ...
        result = result.replace(
            "/data/user/0/com.bloo.android/files/images/photo.jpg",
            "[image] photo.jpg"
        )
